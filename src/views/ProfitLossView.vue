...
import { useIncomeStatementStore } from '@/stores/incomeStatement'
import type { IncomeStatementSection, ReportAccountBalance } from '@/types/reports'

const incomeStatementStore = useIncomeStatementStore()
const { report, loading, error } = storeToRefs(incomeStatementStore)

const now = new Date()
const endDate = ref(formatDateInput(now))
const startDate = ref(formatDateInput(new Date(now.getFullYear(), 0, 1)))
const formError = ref('')
...
interface IncomeSectionView extends IncomeStatementSection {
  accounts: AccountRow[]
}
...
const displayedSections = computed<IncomeSectionView[]>(() => {
...
const summaryCards = computed(() => {
  ...
  return [
    { key: 'REVENUE', label: 'Pendapatan', value: totals?.total_revenue ?? 0 },
    { key: 'EXPENSE', label: 'Beban', value: totals?.total_expenses ?? 0 },
    { key: 'NET', label: 'Laba Bersih', value: totals?.net_income ?? 0 },
  ] as const
})

const periodLabel = computed(() => {
  if (report.value) {
    return `${formatDateLabel(report.value.start_date)} \u2013 ${formatDateLabel(report.value.end_date)}`
  }
  return `${formatDateLabel(startDate.value)} \u2013 ${formatDateLabel(endDate.value)}`
})

const generatedAtLabel = computed(() => (report.value ? formatDateTime(report.value.generated_at) : ''))

const hasData = computed(() =>
  displayedSections.value.some((section) => section.accounts.some((account) => Math.abs(account.balance) > 0.0)),
)
...
function transformSection(section: IncomeStatementSection): IncomeSectionView {
...
async function fetchReport(force = false) {
  try {
    await incomeStatementStore.load(
      {
    ...
    if (report.value?.start_date) {
      startDate.value = report.value.start_date
    }
    if (report.value?.end_date) {
...
async function handleSubmit() {
  ...
  if (startDate.value > endDate.value) {
    formError.value = 'Tanggal mulai tidak boleh setelah tanggal akhir.'
    return
  }
...
<template>
  <div class="space-y-7">
    <header class="space-y-2">
      <h1 class="text-3xl font-semibold text-brand-900">Laporan Laba Rugi</h1>
      <p class="text-sm text-brand-600">Kinerja pendapatan dan beban untuk periode {{ periodLabel }}.</p>
    </header>

    <section class="surface-card space-y-6 p-6">
      <form class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between" @submit.prevent="handleSubmit">
        <div class="flex flex-wrap items-end gap-4">
          <label class="flex flex-col text-sm text-brand-700">
            <span class="text-xs font-semibold uppercase tracking-[0.28em] text-brand-400">Mulai</span>
            <input
              v-model="startDate"
              type="date"
              name="start_date"
          ...
          <label class="flex flex-col text-sm text-brand-700">
            <span class="text-xs font-semibold uppercase tracking-[0.28em] text-brand-400">Selesai</span>
            <input
              v-model="endDate"
              type="date"
              name="end_date"
      ...
      <p v-if="formError" class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700">
        {{ formError }}
      </p>
      ...
      <template v-else-if="report">
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          >
            <p class="text-xs uppercase tracking-[0.28em] text-brand-400">{{ card.label }}</p>
            <p class="mt-2 text-xl font-semibold">{{ formatCurrency(card.value) }}</p>
          </article>
        </div>
        ...
        <div
          ...
          class="rounded-3xl border border-brand-100 bg-brand-50/70 p-6 text-sm text-brand-500"
        >
          Transaksi dengan klasifikasi pendapatan atau beban belum tercatat pada rentang waktu ini.
        </div>
      </template>

      <div v-else class="rounded-3xl border border-brand-100 bg-brand-50/70 p-6 text-sm text-brand-500">
        Belum ada data laba rugi yang dapat ditampilkan.
      </div>
    </section>
  </div>
</template>
